...
public class RedisClusterTest {
    public static void main(String[] args) throws ExecutionException, InterruptedException {
        ...
        RedisConnectionStateListener listener =
                new RedisConnectionStateListener(){
                    ...
                    public void onRedisExceptionCaught(RedisChannelHandler<?, ?> connection, Throwable cause) {
                        log.info("onRedisExceptionCaught:{},exception:{}",
                                connection,cause);
                    }

                    @Override
                    public void onRedisConnected(RedisChannelHandler<?, ?> connection, SocketAddress socketAddress) {
                        log.info("onRedisConnected:{}; sock:{}",connection,
                                socketAddress);
                    }
                };
        ...
        log.info("date 通过keyslot的结果是:{}",
                connection.sync().clusterKeyslot("date"));

        RedisAdvancedClusterReactiveCommands<String,String> async =
                connection.reactive();

        log.info("访问前前的connection是:{}",connection);
        async.set("key", "123")
        .then(async.set("key1","234"))
        .then(async.set("key2","567"))
        .checkpoint();
        ...
        client.shutdown(1,10, TimeUnit.SECONDS);
