 ...
 * <p>
 * 假设 LeetCode 即将开始其 IPO。为了以更高的价格将股票卖给风险投资公司，
 * LeetCode希望在 IPO 之前开展一些项目以增加其资本。 由于资源有限，它只能在 IPO 之前完成最多 k 个不同的项目。
 * 帮助 LeetCode 设计完成最多 k 个不同项目后得到最大总资本的方式。
 ...
 **/
public class IPO {
    volatile ArrayList<Integer> proList = new ArrayList<>();
    volatile ArrayList<Integer> capList = new ArrayList<>();

    public static void main(String[] args) {
        IPO ipo = new IPO();
        int finalProfits = ipo.findMaximizedCapital2(111, 12,
        ...
        System.out.println(finalProfits);
    }

    /**
     * 思路如下，通过两个能够自动排序的队列。其中一个队列(本金)将现有的项目按照由低到高的顺序对项目进行排序
     * 另外一个队列(收益)将现有的收益按照由高到低的顺序进行排序
     * 第一个约束K，表示可以做多少次投资
     * 第二个约束W，表示本金。本金每次循环完会更新。
     * 通过两个队列，先拿本金最少的，再选出本金最少里面收益最高的。
     * 相当于做了两次order by
     * 取他们的前N次和
     * @param k
     * @param W
    ...
    public int findMaximizedCapital2(int k, int W, int[] Profits, int[] Capital) {
        ...
        PriorityQueue<int[]> pqCaptial = new PriorityQueue<>((a, b) -> (a[0] - b[0]));
        //降序,按照收益降序
        PriorityQueue<int[]> pqProfits = new PriorityQueue<>((a, b) -> (b[1] - a[1]));
        for (int i = 0; i < Profits.length; i++) {
            pqCaptial.offer(new int[]{Capital[i], Profits[i]});
        }
        while (k-- != 0) {
            while (!pqCaptial.isEmpty() && pqCaptial.peek()[0] <= W) {
                //把所有的成本小于W的都拿出来，塞到收益降序额profits里面去
                pqProfits.offer(pqCaptial.poll());
            }
        ...
        public int findMaximizedCapital(int k, int W, int[] Profits, int[] Capital) {
            ...
            int maxTimes = Profits.length >= Capital.length ? Capital.length : Profits.length;
            for (int i = 0; i < maxTimes; i++) {
                proList.add(Profits[i]);
                capList.add(Capital[i]);
            }
            while (currentTimes < k) {
                currentTimes++;
                Integer[] idx = findBestCombin(currentW, proList, capList);
                ...
                for (int j = 0; j < idx.length; j++) {
                    perProfits += proList.get(idx[j]);
                    perCaptail += capList.get(idx[j]);
                }
                currentW = currentW + (perProfits - perCaptail);
                finalProfits = finalProfits + perProfits;

                for (int j = 0; j < idx.length; j++) {
                    System.out.println(String.format("j:%s",j));
                    System.out.println(String.format("idx:%s",idx[j]));
                    System.out.println(String.format("before---------profileListSize=[%s], capitalListsize=[%s]",proList.size(),capList.size()));
                    Integer v1 = proList.remove((int) idx[j]);
                    Integer v2 = capList.remove((int) idx[j]);
                    System.out.println(String.format("profileListSize=[%s], capitalListsize=[%s]",proList.size(),capList.size()));
                }
            }
            return finalProfits+W;

        }

        /**
         * 找到利润最高的组合,查找方法，先从成本最低的里面找，如果找到多个，就从收益最高的里面找
         *
         * @param W       当前可用资本
         * @param Profits 最新的收益组合
         * @param Capital 最新的资本组合
         * @return 最新收益组合中，需要选取的index
         */
        private Integer[] findBestCombin(int W, ArrayList<Integer> Profits, ArrayList<Integer> Capital) {

            //第一个是capital 第二个是profits， 第三个是idx
            Map<Integer, Map<Integer, Integer>> matrix = new HashMap<>();
            for (int i = 0; i < Capital.size(); i++) {
                //初始化
                if (matrix.get(Capital.get(i)) == null) {

                    matrix.put(Capital.get(i), new HashMap<>());
                }
                matrix.get(Capital.get(i)).put(Profits.get(i), i);
            }
            int currentW = W;
            //返回值
            List<Integer> idx = new ArrayList<>();
            while (currentW >= 0) {
                int finalCurrentW = currentW;
                List<Integer> captialKeys = matrix.keySet().stream().filter(i -> i <= finalCurrentW).sorted((a, b) -> (a - b)).collect(Collectors.toList());
                //找到成本最低的那个
                if (captialKeys.isEmpty()) {
                    break;
                }
                int cap = captialKeys.get(0);
                Map<Integer, Integer> profitsMap = matrix.get(cap);
                List<Integer> profitsKeys = profitsMap.keySet().stream().sorted((a, b) -> (b - a)).collect(Collectors.toList());
                if (profitsKeys.isEmpty()) {
                    break;
                }
                int pro = profitsKeys.get(0);
                int bestIdx = profitsMap.get(pro);
                idx.add(bestIdx);
                currentW = currentW - cap;
                matrix.get(cap).remove(pro, bestIdx);
                if (matrix.get(cap).isEmpty()) {
                    matrix.remove(cap);
                }
            }


            return idx.toArray(new Integer[0]);
        }
    }
