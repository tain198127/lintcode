...
public class Logistic {
    private static List<LogisticDataSet<Double>> readData(List<String> doc) {
        List<LogisticDataSet<Double>> result;
    ...
    private static List<LogisticDataSet<Double>> readData(String path) {
    ...
    public List<Double> randomGradAscent(List<LogisticDataSet<Double>> dataset, Integer recycle, Double diffAccuracy) {
        ...
        ArrayList<Double> targetX = new ArrayList<>(dataset.get(0).getData().size() + 1);
    ...
    public BigDecimal test(List<LogisticDataSet<Double>> trainData, List<LogisticDataSet<Double>> test) throws FileNotFoundException {
        ...
        return floatErrorCount.divide(new BigDecimal(testDataSize), 4, BigDecimal.ROUND_HALF_UP);
    }
